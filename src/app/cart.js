...
cartRemove(event.target.closest(".product-in-cart").getAttribute("product-in-cart-id"));
...
export const cartGroupObserver = () => {
    const process = () => {
        ...
        // set total
        const cartCostTotal =[...cartGroup.querySelectorAll(".cart-cost")].reduce((pv,cv) => pv +parseFloat( cv.innerText),0)
