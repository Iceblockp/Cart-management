...
import { cartGroup, costTotal, productGroup, searchInput } from "./selectors"
...
export const orderBtnHandler = () => {
    confirmBox(() => {
        ...
        order.products = [...cartGroup.querySelectorAll(".product-in-cart")].map(
            (el) => {
                ...
                productGroup.querySelector(`[product-card-id='${id}']`).querySelector(".add-to-cart-btn").toggleAttribute("disabled");
    ...
    },"Are U sure place order?","This will send to server","question")
}